# 怪物基礎知識（Monster Basics）

來源：《怪物圖鑑》（Monster Manual, MM）
...
The Saving Throws entry is reserved for creatures that are adept at resisting certain kinds of effects. For example, a creature that isn't easily **魅惑** or **恐懼** might gain a bonus on its Wisdom saving throws. Most creatures don't have special saving throw bonuses, in which case this section is absent.
...
The Skills entry is reserved for monsters that are proficient in one or more skills. For example, a monster that is very perceptive and stealthy might have bonuses to Wisdom (**察覺**) and Dexterity (**隱匿**) checks.
...
The Senses entry notes a monster's passive Wisdom (**察覺**) score, as well as any special senses the monster might have. Special senses are described below.

#### Blindsight

A monster with **盲視** can perceive its surroundings without relying on sight, within a specific radius. Creatures without eyes, such as grimlocks and gray oozes, typically have this special sense, as do creatures with echolocation or heightened senses, such as bats and true dragons.

If a monster is naturally blind, it has a parenthetical note to this effect, indicating that the radius of its **盲視** defines the maximum range of its perception.

#### Darkvision

A monster with **黑暗視覺** can see in the dark within a specific radius. The monster can see in dim light within the radius as if it were bright light, and in darkness as if it were dim light. The monster can't discern color in darkness, only shades of gray. Many creatures that live underground have this special sense.
...
A monster with **真實視覺** can, out to a specific range, see in normal and magical darkness, see **隱形** creatures and objects, automatically detect visual illusions and succeed on saving throws against them, and perceive the original form of a shapechanger or a creature that is transformed by magic. Furthermore, the monster can see into the Ethereal Plane within the same range.
...
An innate spell can have special rules or restrictions. For example, a drow mage can innately cast the **浮空術** spell, but the spell has a "self only" restriction, which means that the spell affects only the drow mage.
...
A monster can cast a spell from its list at a higher level if it has the spell slot to do so. For example, a drow mage with the 3rd-level **閃電束** spell can cast it as a 5th-level spell by using one of its 5th-level spell slots.
...
When a monster takes its action, it can choose from the options in the Actions section of its stat block or use one of the actions available to all creatures, such as the **疾走** or **躲藏** action, as described in the *Player's Handbook.*
...
>
> A creature **被擒** by the monster can use its action to try to escape. To do so, it must succeed on a Strength (**運動**) or Dexterity (**體操**) check against the escape DC in the monster's stat block. If no escape DC is given, assume the DC is 10 + the monster's Strength (**運動**) modifier.
>
